...
def cmd_import(engine: ReminiscenceEngine, args) -> int:
    ...
    for f in files:
        try:
            ...
            print(f"[ok] {f.name}: {res.events_created} memory events "
                  f"({res.modality.value})")
...
def cmd_search(engine: ReminiscenceEngine, args) -> int:
    ...
    print(f"classified: {[c.value for c in cq.categories]}"
    ...
    for i, c in enumerate(results, 1):
        ev = c.event
        loc = []
        if ev.page is not None: loc.append(f"p.{ev.page}")
        if ev.section: loc.append(ev.section)
        ...
        print(f"{i:2d}. [{ev.modality.value}] score={c.score:.3f} "
...
def cmd_ask(engine: ReminiscenceEngine, args) -> int:
    ...
    print(json.dumps({"answer": ans.answer, "grounded": ans.grounded,
                      "evidence": ans.evidence, "backend": ans.backend},
...
def cmd_benchmark(engine: ReminiscenceEngine, args) -> int:
    ...
    texts = [f"benchmark sample sentence number {i} about transformers and memory"
             for i in range(64)]

    r1 = runner.bench_fn(
        lambda: engine.embedder.embed(texts),
        model_id=engine.embedder.model_id, task="embedding_batch64",
        input_desc="64 texts x ~10 words", iterations=5,
    )
    q = engine.embedder.embed(["complexity of self attention"])[0]
    r2 = runner.bench_fn(
        lambda: engine.index.search(q, k=10),
        model_id="numpy-ann", task="vector_search",
        input_desc=f"k=10 over {len(engine.index)} vectors", iterations=20,
    )
    r3 = runner.bench_fn(
        lambda: engine.db.fts_search("transformers attention", limit=20),
        model_id="sqlite-fts5", task="lexical_search",
        input_desc="query 'transformers attention'", iterations=20,
    )
    print(runner.report())
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="reminiscence",
                                description="REMINISCENCE — Your Private Multimodal AI Memory")
    ...
    s.add_argument("path"); s.set_defaults(fn=cmd_import)

    s = sub.add_parser("search", help="hybrid memory search")
    s.add_argument("query"); s.add_argument("--top", type=int, default=8)
    ...
    s.add_argument("--days", type=int, default=30); s.set_defaults(fn=cmd_timeline)

    s = sub.add_parser("sources", help="list ingested sources")
    s.set_defaults(fn=cmd_sources)

    s = sub.add_parser("delete", help="explicitly delete a source + memories")
    s.add_argument("source_id"); s.set_defaults(fn=cmd_delete)
